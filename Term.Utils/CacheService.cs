    ...
    /*    public CacheService(MemoryCache memoryCache)
        ...
        */
        ...
        /// <summary>
        /// Gets collections of keys
        /// </summary>
        /// <returns></returns>
        public IEnumerable<string>  GetKeys()
        {
          return   memoryCache.Select(p => p.Key).AsEnumerable();
        }
        /// <summary>
        ...
        /// <param name="beforeItemRemoved">The action to perform before the cache entry has been removed.</param>
        /// <returns>A reference to the cache entry that is identified by key.</returns>
      /*  public T GetOrAdd<U,T>(
            string key,
            Func<U,T> value,
            TimeSpan slidingExpiration,
            Action<string, T> afterItemRemoved = null,
            Action<string, T> beforeItemRemoved = null) where T : class where U : struct, IConvertible
        {
            return this.GetOrAdd(key, value, null, slidingExpiration, afterItemRemoved, beforeItemRemoved);
        }
        */

        public U GetEnumFromString<U,T>(string value) where U : struct, IConvertible  where T:class
        {
            if (!typeof(U).IsEnum)
            {
        ...
        /// <param name="beforeItemRemoved">The action to perform before the cache entry has been removed.</param>
        /// <returns>A reference to the cache entry that is identified by key.</returns>
      /*  private T GetOrAdd<U,T>(
            string key,
            Func<T> getValue,
            DateTimeOffset? absoluteExpiration,
            TimeSpan? slidingExpiration,
            Action<string, T> afterItemRemoved,
            Action<string, T> beforeItemRemoved) where T : class where U:struct,IConvertible
        {
            T value = this.Get<T>(key);
